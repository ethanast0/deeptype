...
export const refreshSession = async (): Promise<{ success: boolean; session?: Session; message?: string; error?: any }> => {
...
export const checkPersistedSession = async (): Promise<Session | null> => {
  try {
...
export const attemptSessionRecovery = async (): Promise<{ success: boolean; session: Session | null; partial?: boolean; error?: any }> => {
  if (sessionRecoveryAttempts >= MAX_RECOVERY_ATTEMPTS) {
    return { success: false, session: null, error: new Error('Max recovery attempts reached') };
  }
  ...
  if (!storedAuth) {
    ...
    return { success: false, session: null, error: new Error('No stored auth data available') };
  }
  ...
  try {
    ...
    return { success: false, session: null, error: new Error('Session recovery failed') };
  } catch (error) {
